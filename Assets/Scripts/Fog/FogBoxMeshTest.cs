...
public class FogBoxMeshTest : MonoBehaviour
{
    public float yMin;
    public float yMax;
    public float radius;
    ...
    public void GenerateMesh ()
    {
        ...
        this.vertices = new Vector3 [verticesPerCircle * this.yResolution];

        // Match range [-1, 1] to [-π/2, π/2] to sample points from sin(x)
        float stepLin = (float) Mathf.PI / this.resolution;
        ...
        for (int yVertexId = 0; yVertexId < this.yResolution; yVertexId ++)
        {
            ...
            int realCircVertexId = 0;
            for (int circVertexId = 0; circVertexId <= this.resolution; circVertexId ++) // n+1 points, with one overlap which is removed
            {
                xVertexRel = Mathf.Sin (xVertexLin) * this.radius;
                zVertexRel = Mathf.Sqrt ( radiusSquared - Mathf.Pow (xVertexRel, 2) );

                // Add vertices (symmetrical shape; prevent vertex overlaps)
                if (circVertexId != this.resolution) {
                    this.vertices [vertIncrement + realCircVertexId] = new Vector3 (xVertexRel, yVertexRel, -1 * zVertexRel); // bottom half of the circle (zVertexRel <= 0)
                    realCircVertexId ++;
                }
                if (circVertexId != 0) {
                    this.vertices [vertIncrement + (verticesPerCircle - realCircVertexId - 1)] = new Vector3 (xVertexRel, yVertexRel, zVertexRel); // top half of the circle (zVertexRel >= 0)
                    realCircVertexId ++;
                }
